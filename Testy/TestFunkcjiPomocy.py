...
class TestFunkcjiPomocyTestCases(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.function_help_test_case = FunctionInfo.FunctionHelpTestCase(self.the_process)
    ...
    def test_funkcji_pomocy(self):
        ...
        while not self.countdown_function_1.finished:
            if first:
                self.function_help_test_case.start()
                first = False
            if self.function_help_test_case.finished:
                break
        Thread.join(self=self.countdown_function_1)
        self.the_log += self.function_help_test_case.this_log
        if not self.function_help_test_case.finished:
            self.function_global.ForTearDown().save_log_to_file(self.test_id, self.the_log)
            self.assertTrue(self.function_help_test_case.finished, self.test_id + ". Brak pokazania sie wszystkich informacji.")
        if "Wyjscie" not in self.function_help_test_case.this_log:
            self.the_log += self.function_global.CommonTestCases().clear_remaining_input(self.the_process)
            self.function_global.ForTearDown().save_log_to_file(self.test_id, self.the_log)
            self.assertIn("Wyjscie", self.function_help_test_case.this_log, self.test_id + ". Brak niezbednych informacji.")
